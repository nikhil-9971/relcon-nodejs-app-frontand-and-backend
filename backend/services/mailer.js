...
function analyzeStatuses(hpclRows, jioRows) {
  ...
  hpclRows.forEach((r) => {
    ...
    const issueType = (safe(r.issueType) || safe(r.purpose) || "").trim().toUpperCase() || "N/A";
  ...
  jioRows.forEach((r) => {
    ...
    const issueType = (safe(r.diagnosis) || safe(r.purpose) || "").trim().toUpperCase() || "N/A";
  ...
  perEngineerRows.sort((a, b) => b.totalVisits - a.totalVisits || a.engineer.localeCompare(b.engineer));
  ...
  perRegionRows.sort((a, b) => b.totalVisits - a.totalVisits || a.region.localeCompare(b.region));

  // Top issues overall (combine HPCL issueType and JIO diagnosis)
  const issueCounts = {};
  hpclRows.forEach((r) => {
    const t = (safe(r.issueType) || safe(r.purpose) || "N/A").trim().toUpperCase();
    issueCounts[t] = (issueCounts[t] || 0) + 1;
  });
  jioRows.forEach((r) => {
    const t = (safe(r.diagnosis) || safe(r.purpose) || "N/A").trim().toUpperCase();
    issueCounts[t] = (issueCounts[t] || 0) + 1;
  });
...
async function sendWeeklyStatusEmail() {
  try {
    ...
    const { perEngineerRows, perRegionRows, topIssues } = analyzeStatuses(hpcl, jio);
    ...
    const attachments = [
      { filename: `Summary_PerEngineer_${start}_to_${end}.csv`, content: toCSV(perEngineerRows, engineerKeys, engineerHeader) },
      ...
      { filename: `HPCL_Detail_${start}_to_${end}.csv`, content: toCSV(
          hpcl.map((r) => ({
            ...r,
    ...
    console.error("❌ Weekly status analysis error:", err.response?.data || err.message);
  }
}
